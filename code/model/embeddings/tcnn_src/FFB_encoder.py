...
"""
    FFBEncoder is based on the paper Fourier Filter Banks
...
from model.embeddings.tcnn_src.hashGridEncoderTcnn import MultiResHashGridEncoderTcnn as MultiResHashGridMLP
...
class FFBEncoder(nn.Module):
    def __init__(self, GridEncoderNetConfig,freq_enc_type,has_out,bound,layers_type,style_modulation=False):
        ...
        self.grid_enc =  MultiResHashGridMLP(include_input = GridEncoderNetConfig['include_input'],
            in_dim = GridEncoderNetConfig['in_dim'],
            network_dims = GridEncoderNetConfig['network_dims'],
            embed_type = 'HashGridTcnn',
            n_levels = GridEncoderNetConfig['n_levels'],
            max_points_per_level = GridEncoderNetConfig['max_points_per_level'],
            log2_hashmap_size = GridEncoderNetConfig['log2_hashmap_size'],
            base_resolution = GridEncoderNetConfig['base_resolution'],
            desired_resolution = GridEncoderNetConfig['desired_resolution'],
            base_sigma = GridEncoderNetConfig['base_sigma'],
            exp_sigma = GridEncoderNetConfig['exp_sigma'],
            grid_embedding_std = GridEncoderNetConfig['grid_embedding_std'],
            per_level_scale = GridEncoderNetConfig['per_level_scale'])
