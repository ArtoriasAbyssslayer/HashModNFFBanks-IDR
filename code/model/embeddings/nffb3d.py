...
class FourierFilterBanks(nn.Module):
    ...
    def __init__(self, GridEncoderNetConfig,has_out,bound):
        ...
        if has_out:
            ...
            if self.include_input:
                self.embeddings_dim = ffenc_dims[-1] * grid_levels - num_inputs
            else:
                self.embeddings_dim = ffenc_dims[-1] * grid_levels
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.has_out:
            if self.include_input:
                ...
                x_out = torch.zero(x.shape[0],self.embeddings_dim,device=input.device)
        else:
            features_list = []
        ...
        for layer in range(0,self.n_ffenc_layers-1):
            ...
            if layer > 0:
                ...
                embed_Feat = embeddings_list[layer-1][:,76:-56] + x
                # Style Modulation #
