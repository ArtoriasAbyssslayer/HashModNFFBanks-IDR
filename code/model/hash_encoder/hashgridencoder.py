...
class _hash_encoder(Function):
    ...
    def forward(ctx,inputs,embeddings,offsets,per_level_scale,base_resolution,calc_grad_input=False):
        ...
        B,D = inputs.shape # batch_size, coord_dims
        L = offsets.shape[0]-1 # level
        C = embeddings.shape[1] # embeddings dim for each chennel
        S = np.log2(per_level_scale)  # resolution multiplier at each level, later CUDA exp2f is applied based on instant ngp
        H = base_resolution 
        
        
        # L first, optimize cache for cuda kernel, but needs an extra permute layer
